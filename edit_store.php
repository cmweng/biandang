...
<div class="container ">
  <div class="row">
    <div class="col-lg-1 py-5 mr-5">
        <div id="list">
          <div class="card border-primary mb-3" style="max-width: 20rem;">
            <div class="card-header"><h5 class="font-weight-bold text-danger">點選要修改的店家</h5></div>
                    ...
                    <table class="table table-hover">
                      <thead>
                        <tr class="text-primary">
                          <th scope="col">#</th>
                          <th scope="col" style="font-weight: bolder;">店家名稱</th>
                          <th scope="col">刪除</th>
                        </tr>
                      </thead>
    ...
    <div class="col-lg-5 mr-5 ml-auto ">
        <div class="py-5">
            <div class="row">
              <div class="col-lg-12">
                <span>店家名稱</span><input class="form-control" type="text" id="store_name" disabled=""><div id="err_store_name"></div>
              </div>
              <div class="col-lg-12">
    ...
    <div class="col-lg-3 py-5 ">
      <div id="btn">
        ...
        <button type="button" class="btn btn-danger" id="ok_btn">輸入資料完成<br>按此按鈕完成修改資料</button>
      </div>             
    </div>
  </div>  
</div>
  ...
  <script>
    ...
    $('#ok_btn').bind("click",function(){
      var arrData = [];
      arrData["store_name"] = $('#store_name').val();
      arrData["store_tel"] = $('#store_tel').val();
      arrData["comment"] = $('#comment').val();
      for (var i=1;i<=91;i++){
        bdname = $('#bdName' +i).val();
        bdprice =$('#bdPrice' +i).val();
        arrData["bdName" + i] = bdname;
        arrData["bdPrice" + i] = bdprice;
      }
      var strData = "";
      var strKey = "";
      ...
      // console.log(strKey);
      $.ajax({
        url:"editstore_api.php",
        type:"post",
        data:{udata:strData, key:strKey},
        success:show,
        error:function(){
          alert("傳送資料發生錯誤!!");
        }
    ...
    function show_storelist(data){
        for (var i = 0; i < data.length; i++){
            strHTML ='<tr><th scope="row">' + (i+1) + '</th><td><a href="#" onclick = "get_data(\''+ data[i].store_name +'\');" >'+ data[i].store_name +'</a></td><td><i class="fas fa-times" onclick=del(\"'+ data[i].store_name +'\");></i></td></tr>';
    ...
    function input(data2){
       ...
       $('#comment').val(data2[0][3]) ;
        for (var i=0;i<=90;i++){
         $('#bdName' +(i+1)).val(data2[0][4+2*i]) ;
         $('#bdPrice' +(i+1)).val(data2[0][4+2*i+1]) ;
    ...
    function del(sname){
      var del = confirm ("確定要刪除 「"+sname+"」 ?");
      console.log(del);
      if(del){
        $.ajax({
          url:"delete_store_api.php",
          type:"post",
          data:{sname:sname},
          success:checktable,
          error:function(){
